  ...
  app.get(markdownPath, async (req, res, next) => {
    try {
      ...
      if (filename.endsWith('.md')) {
        if (!res.locals.user) {
          return res.render('403', { title: 'Forbidden' })
        } else {
          return next()
        }
      }
      ...
      debug('Last:', last, res.locals.user)
      // const lastParts = last.split('.')
      // const ext = lastParts[lastParts.length - 1]
      if (!last.includes('.') || last.endsWith('.draft')) {
        filename += '.md'
      // } else if (last.endsWith('.draft.md')) {
      //   if (res.locals.user && res.locals.user.admin && res.locals.user.admin === true) {
      //     debug('Signed in:', res.locals.user)
      //     filename += ''
      //   } else {
      ...
      // if (!(filename.endsWith('.md') || (!res.locals.user && !filename.endsWith('.draft'))) {
      //
      //   return next()
      // }

      if (last.endsWith('.draft') && !(res.locals.user && res.locals.user.admin && res.locals.user.admin === true)) {
